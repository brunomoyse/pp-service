...
/// Test that status change mutation triggers notifications to all registered players
#[tokio::test]
async fn test_status_change_triggers_notifications() {
    let app_state = setup_test_db().await;
    let schema = build_schema(app_state.clone());

    // Create manager and club
    let (manager_id, manager_claims) =
        create_test_user(&app_state, "status_notif_manager@test.com", "manager").await;
    let club_id = create_test_club(&app_state, "Status Notification Club").await;
    create_club_manager(&app_state, manager_id, club_id).await;

    let tournament_id =
        create_test_tournament(&app_state, club_id, "Status Notification Tournament").await;

    // Create and register players
    let (_, player1_claims) =
        create_test_user(&app_state, "status_notif_player1@test.com", "player").await;
    let (_, player2_claims) =
        create_test_user(&app_state, "status_notif_player2@test.com", "player").await;

    // Register both players (self-registration - don't pass userId)
    ...
    // Update tournament status - should notify both players
    let status_mutation = r#"
        mutation UpdateStatus($input: UpdateTournamentStatusInput!) {
    ...
    let data = response.data.into_json().unwrap();
    assert_eq!(
        data["updateTournamentStatus"]["liveStatus"],
        "REGISTRATION_OPEN"
    );
}
