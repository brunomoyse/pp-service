        ...
        // Log activity and publish achievement notifications
        {
            ...
            tokio::spawn(async move {
                ...
                for (user_id, achievement) in newly_unlocked_achievements {
                    ...
                    // delivery, localized per device. `data.code` lets the app
                    // deep-link to the achievement; best-effort, never blocks
                    // the result entry.
                    crate::services::push_service::send_achievement_unlock(
                        &db,
                        user_id,
                        &achievement.code,
