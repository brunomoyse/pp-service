...
use crate::{AppState, auth::permissions::require_role};
use crate::gql::types::{TournamentClock, TournamentStructure, ClockUpdate, ClockStatus, Role};
use infra::repos::{TournamentClockRepo, ClockStatus as InfraClockStatus};
...
impl TournamentClockQuery {
    ...
    pub async fn tournament_clock(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<Option<TournamentClock>> {
    ...
    pub async fn tournament_structure(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<Vec<TournamentStructure>> {
        let state = ctx.data::<AppState>()?;
        let repo = TournamentClockRepo::new(state.db.clone());
        let tournament_id: Uuid = tournament_id.parse()?;
        ...
        Ok(structures.into_iter().map(|s| TournamentStructure {
            id: s.id.into(),
            tournament_id: s.tournament_id.into(),
            level_number: s.level_number,
            small_blind: s.small_blind,
...
impl TournamentClockMutation {
    ...
    pub async fn create_tournament_clock(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<TournamentClock> {
    ...
    pub async fn start_tournament_clock(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<TournamentClock> {
        let manager = require_role(ctx, Role::Manager).await?;
        let state = ctx.data::<AppState>()?;
        let repo = TournamentClockRepo::new(state.db.clone());
        let tournament_id: Uuid = tournament_id.parse()?;

        let clock_row = repo.start_clock(tournament_id, Some(manager.id.parse()?)).await?;
        let structure = repo.get_current_structure(tournament_id).await.ok();
    ...
    pub async fn pause_tournament_clock(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<TournamentClock> {
        let manager = require_role(ctx, Role::Manager).await?;
        let state = ctx.data::<AppState>()?;
        let repo = TournamentClockRepo::new(state.db.clone());
        let tournament_id: Uuid = tournament_id.parse()?;

        let clock_row = repo.pause_clock(tournament_id, Some(manager.id.parse()?)).await?;
        let structure = repo.get_current_structure(tournament_id).await.ok();
        
        // Calculate time remaining at pause
        let time_remaining = if let (Some(end_time), Some(pause_start)) = (clock_row.level_end_time, clock_row.pause_started_at) {
            let remaining = end_time - pause_start;
            Some(remaining.num_seconds().max(0))
        } else {
            None
    ...
    pub async fn resume_tournament_clock(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<TournamentClock> {
        let manager = require_role(ctx, Role::Manager).await?;
        let state = ctx.data::<AppState>()?;
        let repo = TournamentClockRepo::new(state.db.clone());
        let tournament_id: Uuid = tournament_id.parse()?;

        let clock_row = repo.resume_clock(tournament_id, Some(manager.id.parse()?)).await?;
        let structure = repo.get_current_structure(tournament_id).await.ok();
    ...
    pub async fn advance_tournament_level(&self, ctx: &Context<'_>, tournament_id: ID) -> Result<TournamentClock> {
        let manager = require_role(ctx, Role::Manager).await?;
        let state = ctx.data::<AppState>()?;
        let repo = TournamentClockRepo::new(state.db.clone());
        let tournament_id: Uuid = tournament_id.parse()?;

        let clock_row = repo.advance_level(tournament_id, false, Some(manager.id.parse()?)).await?;
        let structure = repo.get_current_structure(tournament_id).await.ok();
