    ...
    let user_id = match user_row {
        Some(row) => {
            ...
            if let Some(ref password_hash) = row.password_hash {
                ...
                // OAuth-only account: report the same generic error as a bad
                // password so probing can't distinguish account types.
                lockout::record_failure(&form.email);
                return Ok(redirect_with_error(
                    &form.redirect_uri,
                    "access_denied",
        ...
        None => {
            ...
            // lock (and existence isn't revealed by differing behavior). Burn a
            // bcrypt round so the timing matches a real password check.
            PasswordService::verify_dummy(&form.password);
            lockout::record_failure(&form.email);
            return Ok(redirect_with_error(
                &form.redirect_uri,
                "access_denied",
